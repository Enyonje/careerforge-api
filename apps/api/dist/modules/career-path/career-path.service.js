...
let CareerPathService = class CareerPathService {
    ...
    async findCareerPath({ email, userId, }) {
        ...
        user.skills.forEach((us) => userSkillMap.set(us.skillId, us.level));
        ...
        const nextRole = careerPath.find((r) => !r.isEligible);
        let aiRecommendations = null;
        if (nextRole) {
            aiRecommendations = await this.ai.generateRecommendations({
        ...
        return {
            ...
            careerPath,
            recommendedNextRole: nextRole ?? null,
            aiRecommendations,
        };
    }
};
